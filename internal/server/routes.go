...
func (s *Server) SetupRoutes() {
	...
	r.Group(func(r chi.Router) {
		r.Get("/login", s.authService.LoginPageHandler)
	...
	r.Group(func(r chi.Router) {
		...
		r.Route("/tasks", func(r chi.Router) {
			r.Get("/{taskName}", s.taskDetailHandler)
		...
		r.Route("/logs", func(r chi.Router) {
			r.Get("/{executionID}", s.viewLogsHandler)
			r.Get("/{executionID}/download", s.downloadLogsHandler)
			r.Get("/ws/{executionID}", s.logWebSocketHandler)
		})
	})
}
